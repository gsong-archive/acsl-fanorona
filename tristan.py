...
def testIfMoveTo(white, black):
	...
	while a < len(white):
		...
		y = []
		z = []
		if white[a] - 1 not in white and white[a] - 1 not in black and white[a] and white[a] - 1 in range(1, 26):
			white[a] -= 1
			y.append(testIfAdjacent(white, black, a))
			z.append(1)
			if y[0] != None and y[0][0] != a:
				y[0] = None
				z[0] = 0
			white[a] += 1
		else:
		...
		if white[a] + 1 not in white and white[a] + 1 not in black and white[a] and white[a] + 1 in range(1, 26):
			...
			y.append(None)
			z.append(0)
		if white[a] - 5 not in white and white[a] - 5 not in black and white[a] and white[a] - 5 in range(1, 26):
			white[a] -= 5
			y.append(testIfAdjacent(white, black, a))
			z.append(3)
			if y[2] != None and y[2][0] != a:
				y[2] = None
				z[2] = 0
			white[a] += 5
		else:
	...
	while a < 5:
		...
		if len1 == 0 or len1 == -1:
			...
			if moveTo != None:
				...
				n = [0, 0, 0, 0]
				c = movement - 1
				if endUp + setOfMovement[c] in black and endUp + setOfMovement[c] in set1:
					if endUp + 2 * setOfMovement[c] in black and endUp + 2 * setOfMovement[c] in set1:
						if endUp + 3 * setOfMovement[c] in black and endUp + 3 * setOfMovement[c] in set1:
							if endUp + 4 * setOfMovement[c] in black and endUp + 4 * setOfMovement[c] in set1:
								n[c] = 4
							else:
								n[c] = 3
						else:
				...
				d = 0
				while d < 4:
					if n[d] >= n[0] and n[d] >= n[1] and n[d] >= n[2] and n[d] >= n[3]:
						z = d
					d += 1
				...
				while e < 5:
					...
					if possibleList <= 25 and possibleList > 0 and possibleList in black:
